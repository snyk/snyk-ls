...
func TestResetSummaryPanelOnStopScan_CallsInit(t *testing.T) {
	...
	// Seed a folder so workspaceFolderPaths returns something non-empty.
	tmpDir := types.FilePath(t.TempDir())
	_, _ = workspaceutil.SetupWorkspace(t, engine, tmpDir)
	folders := config.GetWorkspace(conf).Folders()
	require.NotEmpty(t, folders, "precondition: workspace must contain at least one folder")

	resetSummaryPanelOnStopScan(agg, conf)

	assert.Len(t, agg.initCalls, 1, "Init must be called once to reset the summary panel")
}

func TestResetSummaryPanelOnStopScan_NilAgg_DoesNotPanic(t *testing.T) {
	engine, _ := testutil.UnitTestWithEngine(t)
	conf := engine.GetConfiguration()
	assert.NotPanics(t, func() {
		resetSummaryPanelOnStopScan(nil, conf)
	})
}

func TestResetSummaryPanelOnStopScan_NoFolders_DoesNotCallInit(t *testing.T) {
	engine, _ := testutil.UnitTestWithEngine(t)
	conf := engine.GetConfiguration()
	// workspace has no folders — Init must not be called
	agg := &initRecordingAggregator{}
	assert.NotPanics(t, func() {
		resetSummaryPanelOnStopScan(agg, conf)
	})
	assert.Empty(t, agg.initCalls, "Init must not be called when there are no workspace folders")
}
