...
func writeSettings(c *config.Config, settings types.Settings, initialize bool) {
	...
	updateToken(settings.Token)             // We update the token before the Authentication method, as the latter checks the token.
	updateAuthenticationMethod(c, settings) // Must happen after the Token update.
