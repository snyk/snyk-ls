...
func Test_GetCodeActions_NoIssues_ReturnsNil(t *testing.T) {
	...
	service := codeaction.NewService(c.Engine(), providerMock, watcher.NewFileWatcher(), notification.NewMockNotifier(), featureflag.NewFakeService(), types.NewConfigResolver(c.Logger()))
...
func setupWithSingleIssue(t *testing.T, c *config.Config, issue types.Issue) (*codeaction.CodeActionsService, types.CodeActionParams, *watcher.FileWatcher) {
	...
	service := codeaction.NewService(c.Engine(), providerMock, fileWatcher, notification.NewMockNotifier(), featureflag.NewFakeService(), types.NewConfigResolver(c.Logger()))
