...
func Test_No_New_Issue_SameRuleId_DifferentContentRoots(t *testing.T) {
	baseContentRoot := types.FilePath("/tmp/snyk_main_abc123")
	currentContentRoot := types.FilePath("/Users/dev/myproject")
	relativePath := "src/main/java/org/example/Main.java"
	...
	baseIssueList := []mockIdentifiable{
		{
			...
			startColumn: 26,
			endColumn:   35,
			path:        baseContentRoot + "/" + types.FilePath(relativePath),
			contentRoot: baseContentRoot,
	...
	currentIssueList := make([]mockIdentifiable, len(baseIssueList))
	for i, base := range baseIssueList {
		currentIssueList[i] = base
		currentIssueList[i].path = currentContentRoot + "/" + types.FilePath(relativePath)
		currentIssueList[i].contentRoot = currentContentRoot
	}
...
func Test_No_New_Issue_DifferentContentRoots(t *testing.T) {
	baseContentRoot := types.FilePath("/tmp/snyk_main_abc123")
	currentContentRoot := types.FilePath("/Users/dev/myproject")
	relativePaths := []types.FilePath{"app.js", "db.js"}
	...
	baseIssueList := []mockIdentifiable{
		{
			...
			startColumn: 10,
			endColumn:   17,
			path:        baseContentRoot + "/" + relativePaths[0],
			contentRoot: baseContentRoot,
			fingerprint: "ae77ea27.4773f344.607187b5.d7919eeb.a1fb1152.5fce695c.fee35010.89d75565.630e4ed1.4773f344.aa4dda5f.d7919eeb.f30fb760.49b28873.85bdc101.83642794",
		},
		{
			...
			startColumn: 10,
			endColumn:   17,
			path:        baseContentRoot + "/" + relativePaths[1],
			contentRoot: baseContentRoot,
			fingerprint: "12567ef6.6d936dbf.bd65d204.fd94bb7c.79a7d027.fcf3002d.81d021f5.91c60b7d.12567ef6.6d936dbf.bd65d204.fd94bb7c.79a7d027.fcf3002d.81d021f5.91c60b7d",
		},
	}
	...
	for i, base := range baseIssueList {
		currentIssueList[i] = base
		currentIssueList[i].path = currentContentRoot + "/" + relativePaths[i]
		currentIssueList[i].contentRoot = currentContentRoot
	}
