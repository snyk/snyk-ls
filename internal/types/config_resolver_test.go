...
func TestConfigResolver_GetValue_MachineScope(t *testing.T) {
	...
	t.Run("returns default source when global string value is empty", func(t *testing.T) {
		...
		assert.Equal(t, configresolver.ConfigSourceDefault, source, "empty string should return ConfigSourceDefault, not ConfigSourceGlobal")
...
func TestConfigResolver_GetValue_OrgScope_DifferentOrgs(t *testing.T) {
	...
	t.Run("uses correct org config based on folder", func(t *testing.T) {
		...
		assert.Equal(t, configresolver.ConfigSourceRemoteLocked, source2)
	})
}
...
func TestFolderConfig_ApplyLspUpdate_RearmsUnmanagedPromptOnDisable(t *testing.T) {
	setup := func(t *testing.T) (*types.FolderConfig, string) {
		t.Helper()
		conf := configuration.NewWithOpts(configuration.WithAutomaticEnv())
		fs := pflag.NewFlagSet("test", pflag.ContinueOnError)
		types.RegisterAllConfigurations(fs)
		...
		fc.ConfigResolver = types.NewMinimalConfigResolver(conf)
		fp := string(types.PathKey(fc.FolderPath))
		// Pre-state: unmanaged was enabled and the user has already been prompted.
		types.SetFolderUserSetting(conf, fc.FolderPath, types.SettingSnykOssUnmanagedEnabled, true)
		types.SetFolderUserSetting(conf, fc.FolderPath, types.SettingSnykOssUnmanagedPrompted, true)
		return fc, fp
	}

	t.Run("setting unmanaged_enabled=false clears the prompted latch", func(t *testing.T) {
		fc, _ := setup(t)
		require.True(t, types.HasUserOverride(fc.Conf(), fc.FolderPath, types.SettingSnykOssUnmanagedPrompted),
			"prompted latch should be set before update")

		update := &types.LspFolderConfig{
			FolderPath: "/path/to/folder",
			Settings: map[string]*types.ConfigSetting{
				types.SettingSnykOssUnmanagedEnabled: {Value: false, Changed: true},
			},
		}

		assert.True(t, fc.ApplyLspUpdate(update))
		assert.False(t, types.HasUserOverride(fc.Conf(), fc.FolderPath, types.SettingSnykOssUnmanagedPrompted),
			"prompted latch should be cleared so the next scan re-asks if C/C++ files are present")
	})

	t.Run("resetting unmanaged_enabled to default also clears the prompted latch", func(t *testing.T) {
		fc, _ := setup(t)

		update := &types.LspFolderConfig{
			FolderPath: "/path/to/folder",
			Settings: map[string]*types.ConfigSetting{
				types.SettingSnykOssUnmanagedEnabled: {Value: nil, Changed: true},
			},
		}

		fc.ApplyLspUpdate(update)
		assert.False(t, types.HasUserOverride(fc.Conf(), fc.FolderPath, types.SettingSnykOssUnmanagedPrompted),
			"reset to default (false) should also clear the latch")
	})

	t.Run("setting unmanaged_enabled=true preserves the prompted latch", func(t *testing.T) {
		fc, _ := setup(t)

		update := &types.LspFolderConfig{
			FolderPath: "/path/to/folder",
			Settings: map[string]*types.ConfigSetting{
				types.SettingSnykOssUnmanagedEnabled: {Value: true, Changed: true},
			},
		}

		fc.ApplyLspUpdate(update)
		assert.True(t, types.HasUserOverride(fc.Conf(), fc.FolderPath, types.SettingSnykOssUnmanagedPrompted),
			"enabling unmanaged should not re-arm the prompt (it's already in effect)")
	})

	t.Run("Changed=false is ignored (no spurious clears)", func(t *testing.T) {
		fc, _ := setup(t)

		update := &types.LspFolderConfig{
			FolderPath: "/path/to/folder",
			Settings: map[string]*types.ConfigSetting{
				types.SettingSnykOssUnmanagedEnabled: {Value: false, Changed: false},
			},
		}

		fc.ApplyLspUpdate(update)
		assert.True(t, types.HasUserOverride(fc.Conf(), fc.FolderPath, types.SettingSnykOssUnmanagedPrompted),
			"Changed=false indicates the IDE did not modify the setting; latch must not be touched")
	})
}
...
func TestInteg_ConfigResolution_WriteFolderConfig_RoundTrip(t *testing.T) {
	...
	assert.Equal(t, configresolver.ConfigSourceRemote, source2)
...
func TestInteg_OrgPrecedence_MultiFolderDifferentOrgs_IndependentResolution(t *testing.T) {
	...
	assert.Equal(t, configresolver.ConfigSourceRemote, src1)
	assert.Equal(t, false, val2, "locked org remote should block user override in folder2")
	assert.Equal(t, configresolver.ConfigSourceRemoteLocked, src2)
}
...
func TestInteg_FolderPrecedence_MultiFolderIsolation(t *testing.T) {
	...
	assert.Equal(t, configresolver.ConfigSourceUserFolderOverride, src1)
	assert.Equal(t, "global-main", val2, "folder2 should fall back to user global")
	assert.Equal(t, configresolver.ConfigSourceUserGlobal, src2)
}
