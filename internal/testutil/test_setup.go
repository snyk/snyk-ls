...
func OnlyEnableCode(t *testing.T, engine workflow.Engine) {
	...
	conf := engine.GetConfiguration()
	logger := engine.GetLogger()
	types.SetGlobalDeferredFolderScope(conf, types.SettingSnykIacEnabled, false)
	types.SetGlobalDeferredFolderScope(conf, types.SettingSnykOssEnabled, false)
	types.SetGlobalDeferredFolderScope(conf, types.SettingSnykCodeEnabled, true)
