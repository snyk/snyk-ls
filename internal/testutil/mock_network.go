...
func SetupEngineMockWithNetworkAccess(t *testing.T) (*mocks.MockEngine, configuration.Configuration) {
	...
	// Mock GetLogger - return a nop logger for tests
	nopLogger := zerolog.Nop()
	mockEngine.EXPECT().GetLogger().Return(&nopLogger).AnyTimes()

	// Mock SetLogger (called during config initialization)
	mockEngine.EXPECT().SetLogger(gomock.Any()).AnyTimes()

	// Mock GetWorkflows (called by analytics) - returns empty slice of workflow URLs
	mockEngine.EXPECT().GetWorkflows().Return([]*url.URL{}).AnyTimes()

	// Mock InvokeWithInputAndConfig (called by analytics) - return nil to silently ignore analytics
