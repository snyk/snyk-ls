...
(function () {
	...
	formState.attachFormStateListeners = function () {
		...
		// Disable the CLI path field while binaries are managed automatically
		var automaticDownload = dom.getByName("automatic_download")[0];
		var cliPath = dom.get("cli_path");
		if (automaticDownload && cliPath) {
			var syncCliPathDisabled = function () {
				cliPath.disabled = automaticDownload.checked;
			};
			dom.addEvent(automaticDownload, "change", syncCliPathDisabled);
