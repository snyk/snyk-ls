...
func TestPlaceholderRemediationAPI_GetBaseImageRecommendation(t *testing.T) {
	api := &PlaceholderRemediationAPI{}
	ctx := t.Context()

	recommendation, err := api.GetBaseImageRecommendation(ctx, "ubuntu:18.04")

	require.NoError(t, err)
	assert.Equal(t, "ubuntu:18.04", recommendation.OriginalImage)
	assert.Equal(t, "ubuntu:22.04-slim", recommendation.RecommendedImage)
