...
CMD ["nginx", "-g", "daemon off;"]`
	...
	for _, issue := range issues {
		...
		// Track which base images were found
		if issue.GetID() == "SNYK-CONTAINER-BASE-IMAGE-ubuntu:18.04" {
			baseImagesFound["ubuntu:18.04"] = true
		} else if issue.GetID() == "SNYK-CONTAINER-BASE-IMAGE-node:14" {
			baseImagesFound["node:14"] = true
		} else if issue.GetID() == "SNYK-CONTAINER-BASE-IMAGE-nginx:1.19-alpine" {
...
CMD ["python", "app.py"]`
	...
	issue := issues[0]
	assert.Equal(t, "SNYK-CONTAINER-BASE-IMAGE-python:3.8", issue.GetID())
...
CMD ["node", "server.js"]`
	...
	issue := issues[0]
	assert.Equal(t, "SNYK-CONTAINER-BASE-IMAGE-alpine:3.14", issue.GetID())
...
ENTRYPOINT ["/app"]`
	...
	issue := issues[0]
	assert.Equal(t, "SNYK-CONTAINER-BASE-IMAGE-golang:1.19", issue.GetID())
