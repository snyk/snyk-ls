...
// newNoLessonsLearnMock returns a mock learn.Service that responds to any GetLesson call
// with an empty Lesson and no error. Mirrors infrastructure/code/code_test.go:setupMockLearnServiceNoLessons
// so the secrets and code packages share an identical no-lesson stub shape.
func newNoLessonsLearnMock(t *testing.T) *mock_learn.MockService {
	t.Helper()
...
func TestScanner_Scan_UsesConfigResolverFromContext(t *testing.T) {
	...
	scanner := New(engine.GetConfiguration(), engine, engine.GetLogger(), performance.NewInstrumentor(), &snyk_api.FakeApiClient{}, newNoLessonsLearnMock(t), featureflag.NewFakeService(), notification.NewMockNotifier(), defaultResolver(engine))
...
func TestScanner_Scan_FallsBackToStructFieldWhenNoResolverInContext(t *testing.T) {
	...
	workspaceFolder := types.FilePath(t.TempDir())
	scanner := New(engine.GetConfiguration(), engine, engine.GetLogger(), performance.NewInstrumentor(), &snyk_api.FakeApiClient{}, newNoLessonsLearnMock(t), featureflag.NewFakeService(), notification.NewMockNotifier(), mockResolver)
...
func TestScanner_Scan(t *testing.T) {
	t.Run("returns issues from findings", func(t *testing.T) {
		...
		workspaceFolder := types.FilePath(t.TempDir())
		scanner := New(mockConf, mockEngine, engine.GetLogger(), performance.NewInstrumentor(), &snyk_api.FakeApiClient{}, newNoLessonsLearnMock(t), featureflag.NewFakeService(), notification.NewMockNotifier(), defaultResolver(mockEngine))
...
// TestScanner_Scan_PopulatesLessonUrl verifies that after a successful scan the secrets
// scanner enhances each issue with the lesson URL returned by the learn service. Mirrors
// infrastructure/code/code.go:enhanceIssuesDetails behavior for the secrets product.
func TestScanner_Scan_PopulatesLessonUrl(t *testing.T) {
	engine := testutil.UnitTest(t)
	mockEngine, mockConf := testutil.SetUpEngineMock(t, engine)
	mockConf.Set(configresolver.UserGlobalKey(types.SettingSnykSecretsEnabled), true)
	...
	rule := newSecretsRuleProblem("hardcoded-secret", "Hardcoded Secret", []string{"Security"})
	finding := newFinding(
		"populate-key", "Hardcoded Secret Found", "A hardcoded secret was detected",
		testapi.SeverityHigh, []testapi.FindingLocation{loc}, []testapi.Problem{cwe, rule}, nil,
	)
	...
	workspaceFolder := types.FilePath(t.TempDir())
	scanner := New(mockConf, mockEngine, engine.GetLogger(), performance.NewInstrumentor(), &snyk_api.FakeApiClient{}, learnMock, featureflag.NewFakeService(), notification.NewMockNotifier(), defaultResolver(mockEngine))
	ctx := ctx2.NewContextWithFolderConfig(t.Context(), secretsEnabledFolderConfig(workspaceFolder))
...
// TestScanner_Scan_LessonUrlEmptyOnLearnError verifies that errors from the learn service
// do not propagate or fail the scan; LessonUrl simply stays empty for the affected issues.
func TestScanner_Scan_LessonUrlEmptyOnLearnError(t *testing.T) {
	engine := testutil.UnitTest(t)
	mockEngine, mockConf := testutil.SetUpEngineMock(t, engine)
	mockConf.Set(configresolver.UserGlobalKey(types.SettingSnykSecretsEnabled), true)
	...
	cwe := newCweProblem("CWE-798")
	finding := newFinding(
		"err-key", "Hardcoded Secret Found", "A hardcoded secret was detected",
	...
	learnMock.EXPECT().
		GetLesson(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), types.SecretsIssue).
		Return(nil, errors.New("learn cache miss")).
		Times(1)
	...
	require.NoError(t, err)
	require.Len(t, issues, 1)
	assert.Empty(t, issues[0].GetLessonUrl(), "scan must succeed and LessonUrl stays empty on learn error")
}

// TestScanner_Scan_LessonUrlEmptyOnNilLesson verifies the cache-miss path: a nil lesson
// returned from the learn service must not be set on the issue.
func TestScanner_Scan_LessonUrlEmptyOnNilLesson(t *testing.T) {
	engine := testutil.UnitTest(t)
	mockEngine, mockConf := testutil.SetUpEngineMock(t, engine)
	mockConf.Set(configresolver.UserGlobalKey(types.SettingSnykSecretsEnabled), true)
	...
	cwe := newCweProblem("CWE-798")
	finding := newFinding(
		"nil-key", "Hardcoded Secret Found", "A hardcoded secret was detected",
		testapi.SeverityHigh, []testapi.FindingLocation{loc}, []testapi.Problem{cwe}, nil,
	)
