...
Canonical notes for **megaproject-scale** runs of `Test_SmokeRealScanMonorepoFixture` (`application/server/server_smoke_test.go`): wall time, CPU profile shape, heap snapshots, and how that compares across work sessions. Implementation-plan detail for checkpoints **cp8** and branch work lives in the repo’s local IDE-1940 plan; this file is **versioned** under `docs/` (plan filenames matching the gitignore pattern `*IDE-*` stay untracked).

## Fixture and harness (all sessions below)

- **Fixture:** `BENCHMARK_REALSCAN_FULL_FIXTURE=1` — 500 generated workspace leaves whose names match `code_*`, plus 500 matching `oss_*` (1000 total). When `code+oss leaves > 100`, the harness **skips per-leaf `publishDiagnostics` assertions** but still runs real Code + OSS work.
...
1. **`BoltBackend.Get(path)`** (`infrastructure/issuecache/backend/bolt_backend.go`): each call runs a **bbolt read transaction**, loads **one KV value** (JSON array of issues for that path), and runs **`json.Unmarshal`** into a slice of `*snyk.Issue`. There is **no in-process decode cache**: every **`IssueCache.IssuesForFile`** / **`IssuesForRange`** / **`Issue(...)`** that hits the backend **re-decodes the entire array** for that file.
...
- **Reduce decode frequency:** process-level LRU of decoded slices of `types.Issue` keyed by path and generation (invalidate on `Set` / `ClearIssues` for that path), or ensure high-level callers **batch** file reads instead of N× redundant `IssuesForFile` for the same path in one tick.
...
- **Instrumentation first:** count calls and bytes for `BoltBackend.Get`, `Set`, and `unmarshalIssues` per scan phase to see whether the hotspot is **repeated reads** vs **one-shot giant unmarshals**.
...
- `AddSnykLearnAction` (`infrastructure/oss/code_actions.go`): process-global `learnCodeActionLookupCache` keys on `(packageManager, vulnId, first CWE, first CVE)` — aligned with `learn.Service` lookup — and stores only whether a lesson URL exists plus the URL; the display title (“Learn more about …”) is **rebuilt** on each hit from the caller’s current vuln title. `GetLesson` errors are not cached. A **negative** result (no lesson / empty URL) is cached until process restart (same trade-off class as other memoization).
...
- **Profiling reports:** when publishing numbers, pair pprof `inuse_space` with `MemStats` peaks (`HeapSys`, `HeapInuse`) so readers do not mix **live heap objects** (pprof snapshot) with **allocator arena pressure** (OS-facing `HeapSys`).
