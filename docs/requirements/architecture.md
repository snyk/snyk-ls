...
**Context.** An earlier draft bundled the per-server progress channel + token→tracker registry into a value named `*progress.Bus`. The PR author rejected "Bus" and requires the per-server owner be **surfaced as `progress.Tracker`**. This collides with the EXISTING `progress.Tracker` type, which today is the per-operation progress handle (token, cancel channel, ~280 lines of `ui.ProgressBar` methods: Begin/Report/End/Clear/CancelOrDone). The question is which name binds to which responsibility.
...
type Task struct {
    tracker       *Tracker            // back-reference so Clear/CancelOrDone deregister without a global
...
// CancelOrDone, GetToken/GetChannel/GetCancelChannel, IsCanceled (now t.tracker.IsCanceled(t.token)).
// deleteTracker → t.tracker.delete(t.token). Self-cancel in code.go uses t.tracker.Cancel(t.token)
...
- `ls_extension/language_server_workflow.go:110` — GAF extension-mode UI tracker: `user_interface.WithProgressBar(progress.NewTracker(true, logger))`. One GAF engine per process, wired before `server.Start`. Resolved by wiring it through to the server's own owner: the UI installation moved into `server.Start`, immediately after `deps := di.Init(...)`, and mints its bars from `deps.ProgressTracker` — the channel `createProgressListener` drains. No standalone owner remains, so no `APPROVED-KEEP` annotation is needed. Bars are minted per `NewProgressBar()` call and are not cancellable: nothing handles cancellation for the GAF UI bar.
...
| `progress.Bus` (earlier draft name) | Author explicitly rejected the name "Bus". |
