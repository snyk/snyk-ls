...
- Optional: `BENCHMARK_ISSUE_CACHE_BACKEND=memory` (`testsupport.BenchmarkIssueCacheBackendEnvVar`) — before `di.Init`, overrides the default **`issue_cache_backend`** so **Snyk Code** and **Secrets** use in-memory imcache instead of **bolt** (the LS default since IDE-1940 cp11r.11). Set to `bolt` or `disk` only if you need to match an explicit value. Megaproject **heap / alloc** measurements should normally use the default on-disk path. **OSS** and **IaC** use `issuecache.IssueCache` (bolt by default per `issue_cache_backend`; IaC since IDE-1940 **cp25**).
