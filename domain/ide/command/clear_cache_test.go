...
func Test_ClearCache_DeleteAll_NoError(t *testing.T) {
	...
	w := workspace.New(c, performance.NewInstrumentor(), sc, nil, scanNotifier, notification.NewMockNotifier(), scanPersister, scanStateAggregator, featureflag.NewFakeService(), types.NewConfigResolver(nil, c, nil))
	folder := workspace.NewFolder(c, "dummy", "dummy", sc, nil, scanNotifier, notification.NewMockNotifier(), scanPersister, scanStateAggregator, featureflag.NewFakeService(), types.NewConfigResolver(nil, c, nil))
	w.AddFolder(folder)
	c.SetWorkspace(w)
