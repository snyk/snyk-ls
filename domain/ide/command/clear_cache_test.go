...
func Test_ClearCache_DeleteAll_NoError(t *testing.T) {
	...
	w := workspace.New(c.Engine().GetConfiguration(), c.Logger(), performance.NewInstrumentor(), sc, nil, scanNotifier, notification.NewMockNotifier(), scanPersister, scanStateAggregator, featureflag.NewFakeService(), resolver)
	folder := workspace.NewFolder(c.Engine().GetConfiguration(), c.Logger(), types.PathKey("dummy"), "dummy", sc, nil, scanNotifier, notification.NewMockNotifier(), scanPersister, scanStateAggregator, featureflag.NewFakeService(), resolver)
	w.AddFolder(folder)
	c.SetWorkspace(w)
