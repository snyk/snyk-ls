...
// TestHandleFoldersDoesNotPanicOnCanceledContext (IDE-2036-UNIT-101) verifies
// that HandleFolders accepts an already-canceled context without panicking.
//
// That HandleFolders actually forwards the context to the scan goroutines is
// covered by TestHandleFoldersScanCtxCanceledOnShutdown (IDE-2036-INTEG-101).
func TestHandleFoldersDoesNotPanicOnCanceledContext(t *testing.T) {
	engine := testutil.UnitTest(t)
