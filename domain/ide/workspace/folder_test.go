...
func Test_filterByIsNew(t *testing.T) {
	...
	newIssue := &snyk.Issue{ID: "new", IsNew: true, Product: product.ProductCode}
	oldIssue := &snyk.Issue{ID: "old", IsNew: false, Product: product.ProductCode}
	...
	t.Run("returns only new issues and counts the rest as dropped", func(t *testing.T) {
		filtered, dropped := f.filterByIsNew(snyk.IssuesByFile{filePath: {newIssue, oldIssue}})
	...
	t.Run("nil issues are skipped without being counted as dropped", func(t *testing.T) {
		filtered, dropped := f.filterByIsNew(snyk.IssuesByFile{filePath: {newIssue, nil, oldIssue}})
...
func Test_filterByIsNew_KeepsIssuesOfProductsWithoutBaseline(t *testing.T) {
	...
	result, _ := f.filterByIsNew(snyk.IssuesByFile{filePath: {noBaselineIssue}})
