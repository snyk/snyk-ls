...
func TestChangeWorkspaceFolders_SendsRefresh(t *testing.T) {
	engine := testutil.UnitTest(t)
	conf := engine.GetConfiguration()
	logger := engine.GetLogger()
	sc := &scanner.TestScanner{}
	...
	w := New(conf, logger, performance.NewInstrumentor(), sc, nil, scanNotifier, mockNotifier, nil, scanStateAggregator, featureflag.NewFakeService(), defaultResolver(engine), engine)
	...
	params := types.DidChangeWorkspaceFoldersParams{Event: types.WorkspaceFoldersChangeEvent{
		Added: []types.WorkspaceFolder{
			{Name: "newFolder", Uri: uri.PathToUri("newFolder")},
		},
	}}

	w.ChangeWorkspaceFolders(params)

	sent := mockNotifier.SentMessages()
	found := false
	...
	assert.True(t, found, "expected RefreshHtmlSettingsParams to be sent when folder is added")
}

func TestChangeWorkspaceFolders_NoChange_NoRefresh(t *testing.T) {
	engine := testutil.UnitTest(t)
	conf := engine.GetConfiguration()
	logger := engine.GetLogger()
	sc := &scanner.TestScanner{}
	...
	for _, msg := range mockNotifier.SentMessages() {
		if _, ok := msg.(types.RefreshHtmlSettingsParams); ok {
			t.Error("RefreshHtmlSettingsParams must not be sent when no folders changed")
		}
	}
}

func TestChangeWorkspaceFolders_NotInitialized_NoRefresh(t *testing.T) {
	engine := testutil.UnitTest(t)
	conf := engine.GetConfiguration()
	logger := engine.GetLogger()
	sc := &scanner.TestScanner{}
	...
	for _, msg := range mockNotifier.SentMessages() {
		if _, ok := msg.(types.RefreshHtmlSettingsParams); ok {
			t.Error("RefreshHtmlSettingsParams must not be sent when LSP is not initialized")
		}
	}
}
