...
test("clicking .reset-global-overrides-btn saves 14 top-level nulls immediately once confirmed", async () => {
	const win = await buildDom();

	// The reset is gated behind ideBridge.confirm (IDE-1945). With no IDE bridge in
	// JSDOM it falls back to window.confirm, which JSDOM does not implement — stub it
	// to auto-accept so the confirm callback fires with true and the reset proceeds.
	win.confirm = () => true;
	...
	// On confirm, the click triggers the save immediately via getAndSaveIdeConfig().
	btn.click();
...
test("else branch (no autoSave) clears both globalReset and folderResets marks", async () => {
	...
	// Reset is gated behind ideBridge.confirm (IDE-1945); stub the window.confirm
	// fallback to auto-accept so the handler body (and its else branch) runs.
	win.confirm = () => true;
