...
/**
 * Builds a fully-initialized JSDOM environment from the settings-fallback HTML fixture.
 * The fixture is generated by: go run scripts/settings-fallback-fixture/main.go
 * Field values are already embedded via placeholder substitution in the fixture.
 *
 * @param {object} [overrides] Optional DOM field overrides applied before load fires.
 * @param {string} [overrides.fixtureName="settings-fallback.html"] Fixture file name.
 * @returns {Promise<Window>}
 */
export async function buildFallbackDom({ fixtureName = "settings-fallback.html" } = {}) {
  const html = await readFile(join(__dirname, "fixtures", fixtureName), "utf8");
  const dom = new JSDOM(html, { runScripts: "dangerously" });
  const win = dom.window;

  // Yield to the event loop so JSDOM fires the window 'load' event,
  // which runs captureInitialState() and attaches all listeners.
  await new Promise((resolve) => win.setTimeout(resolve, 0));
